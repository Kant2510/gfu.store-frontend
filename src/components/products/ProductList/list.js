...
const sortProduct = (option, game_data) => {
    ...
    //
    var final_game_data = game_data.filter(
        (ele) =>
            ele.category === (option.category.action === true ? 'Action' : '')
    )
    final_game_data = final_game_data.concat(
        game_data.filter(
            (ele) =>
                ele.category ===
                (option.category.adventure === true ? 'Adventure' : '')
        )
    )
    final_game_data = final_game_data.concat(
        game_data.filter(
            (ele) =>
                ele.category === (option.category.fps === true ? 'FPS' : '')
        )
    )
    final_game_data = final_game_data.concat(
        game_data.filter(
            (ele) =>
                ele.category ===
                (option.category.horror === true ? 'Horror' : '')
        )
    )
    final_game_data = final_game_data.concat(
        game_data.filter(
            (ele) =>
                ele.category ===
                (option.category.intelligence === true ? 'Intelligence' : '')
        )
    )
    final_game_data = final_game_data.concat(
        game_data.filter(
            (ele) =>
                ele.category ===
                (option.category.multiplayer === true ? 'Multiplayer' : '')
        )
    )
    final_game_data = final_game_data.concat(
        game_data.filter(
            (ele) =>
                ele.category === (option.category.sport === true ? 'Sport' : '')
        )
    )
    //
    if (option.range.begin !== 0 || option.range.end !== 0) {
        final_game_data = final_game_data.filter((ele) => {
            ...
            if (option.range.end === 0) {
                ...
                return (
                    gamePrice >= option.range.begin &&
                    gamePrice <= option.range.end
                )
        })
    }
...
export default function List({ option }) {
    ...
    const game_data = sortProduct(option, productsData)
    return (
        <div>
